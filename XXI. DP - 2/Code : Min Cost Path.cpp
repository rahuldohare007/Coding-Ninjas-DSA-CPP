...
Input format :
The first line of the test case contains two integer values, 'M' and 'N', separated by a single space. They represent the 'rows' and 'columns' respectively, for 
the two-dimensional array/list.
...
int minCostPathHelper(int **input, int m, int n, int i, int j, int **output){
    ...
    int downCost = minCostPathHelper(input, m, n, i + 1, j, output);
    int diagonalCost = minCostPathHelper(input, m, n, i + 1, j + 1, output);
    int leftCost = minCostPathHelper(input, m, n, i, j + 1, output);
    
    output[i][j] = input[i][j] + min(diagonalCost, min(downCost, leftCost));
